...
(function () {
    ...
    function init() {
        ...
        let buttonhtml = `<div id="ExternalPlayersBtns" class ="detailButtons flex align-items-flex-start flex-wrap-wrap">
            ...
            <button id="embyDandanplay" type="button" class="detailButton  emby-button emby-button-backdropfilter raised-backdropfilter detailButton-primary" title="弹弹Play"> <div class="detailButton-content"> <i class="md-icon detailButton-icon button-icon button-icon-left icon-DandanPlay">　</i>  <span class="button-text">弹弹Play</span> </div> </button>
    ...
    async function embyDandanplay() {
        let mediaInfo = await getEmbyMediaInfo();
        let intent = mediaInfo.intent;
        let DandanplayUrl = null;
        if (getOS() == "android") {
            DandanplayUrl = `intent:${encodeURI(mediaInfo.streamUrl)}#Intent;package=com.xyoye.dandanplay;type=video/*;end`;
        } else {
            DandanplayUrl = 'ddplay:' + encodeURIComponent(`${mediaInfo.streamUrl}|filePath=${intent.title}`);
        }
        console.log(`DandanplayUrl= ${DandanplayUrl}`);
        window.open(DandanplayUrl, "_blank");
    }
